...
1. cache_control: 如果不设置, 则上游必须有 cache 相关参数, 否则不能缓存(Bypass)
2. content_type: 必须是完全匹配(`application/json; charset=utf-8`)
3. cache key
   - key = md5(UUID | method | request{vary_headers/vary_query_params/xxx})
4. 不能在同一路由下只缓存单个 API: 需要分开创建路由后统一缓存
5. 在 Kong 核心请求处理模型中的实现, 此时当上游无法访问时, 代理缓存插件无法用于提供过时的缓存数据: 定义一个非常大的 storage_ttl(小时/天), 以便在缓存中保留过时的数据
6. storage_ttl / cache_ttl
